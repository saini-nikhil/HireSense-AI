...
type SectionKey = "experience" | "projects" | "education";
...
function resumeTemplate(data: ResumeForm): string {
...
export default function AtsResumeBuilder() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm({ ...form, [e.target.name]: e.target.value } as ResumeForm);
  ...
  const handleNestedChange = (
    ...
    updated[index] = { ...(updated[index] as object), [field]: value } as (typeof updated)[number];
    setForm({ ...form, [section]: updated } as ResumeForm);
  };

  const addItem = (section: SectionKey, template: ResumeForm[SectionKey][number]) =>
    setForm({ ...form, [section]: [...form[section], template] } as ResumeForm);

  const removeItem = (section: SectionKey, index: number) => {
    if (form[section].length === 1) return;
    const updated = form[section].filter((_, i: number) => i !== index);
    setForm({ ...form, [section]: updated } as ResumeForm);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(backendUrl("/users/resumebuilder"), {
        method: "POST",
      ...
      <div className="w-full lg:w-1/2 overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            ...
            {step === 2 && (
              <div className="space-y-4">
                {form.experience.map((exp, i: number) => (
            ...
            {step === 3 && (
              <div className="space-y-4">
                {form.projects.map((proj, i: number) => (
                  <div key={i} className={sectionCardClass}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                        Project {i + 1}
            ...
            {step === 4 && (
              <div className="space-y-4">
                {form.education.map((edu, i: number) => (
                  <div key={i} className={sectionCardClass}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                        Degree {i + 1}
