...
type Analysis = {
  ...
  suggestions: string[];
  resumeImprovements: { section: string; issue: string; fix: string }[];
};
...
function ScoreRing({
  ...
  return (
    ...
    >
      ...
      >
        <circle cx="29" cy="29" r="26" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="5" />
        <circle
          cx="29" cy="29" r="26" fill="none"
          stroke={color} strokeWidth="5" strokeLinecap="round"
          strokeDasharray={CIRC} strokeDashoffset={offset}
          style={{ animation: `spinIn 1.2s ease ${delay}s both` }}
        />
        <text
          x="29" y="34" textAnchor="middle" fontSize="11" fontWeight="500" fill="#e2e8f0"
...
// ── Pill tag ─────────────────────────────────────────────────────────
function Pill({ text, variant }: { text: string; variant: "green" | "red" | "purple" }) {
  const styles = {
  ...
  return (
    <span className={`inline-block px-3 py-1 rounded-full text-xs m-1 ${styles[variant]}`}>
      {text}
    </span>
  );
}
...
function DifficultyBadge({ level }: { level: string }) {
  const map: Record<string, string> = {
    easy:   "bg-emerald-400/10 text-emerald-400 border border-emerald-400/25",
    medium: "bg-amber-400/10 text-amber-400 border border-amber-400/25",
    hard:   "bg-red-400/10 text-red-400 border border-red-400/25",
  };
  return (
    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-wide ${map[level] ?? map.medium}`}>
      {level}
    </span>
  );
}
...
function TypeBadge({ type }: { type: string }) {
  const map: Record<string, string> = {
    technical:   "bg-blue-400/10 text-blue-400 border border-blue-400/25",
    behavioural: "bg-violet-400/10 text-violet-400 border border-violet-400/25",
    situational: "bg-pink-400/10 text-pink-400 border border-pink-400/25",
  };
  return (
    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${map[type] ?? "bg-slate-400/10 text-slate-400"}`}>
      {type}
    </span>
  );
}
...
function QuestionCard({
  ...
  q: { type: string; difficulty: string; question: string; what_to_listen_for: string };
  index: number;
}) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-slate-600 text-xs w-5 flex-shrink-0 mt-0.5">
      ...
      {/* Expandable "what to listen for" */}
      {open && (
        <div
          className="px-4 pb-3.5 pt-0 text-xs text-slate-500 leading-relaxed border-t border-white/[0.05]"
          style={{ borderTop: "0.5px solid rgba(255,255,255,0.06)" }}
        >
          <span className="text-violet-400 font-medium mr-1">What to listen for:</span>
...
function ImprovementCard({
  ...
  item: { section: string; issue: string; fix: string };
}) {
  const [open, setOpen] = useState(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-slate-400 flex-1 min-w-0 truncate">{item.issue}</p>
        <span
      ...
      {open && (
        <div
          className="px-4 pb-3.5 text-xs text-slate-400 leading-relaxed"
          style={{ borderTop: "0.5px solid rgba(255,255,255,0.06)" }}
        >
          <p className="text-amber-400/70 font-medium mb-1 pt-2">✦ Suggested fix</p>
          {item.fix}
        </div>
      )}
    </div>
      ...
      {(
        ...
        <div
          ...
          style={{
            ...
            background: o.c, filter: "blur(70px)", opacity: 0.18,
            top: o.t, left: o.l,
            bottom: o.b, right: o.r,
...
export default function ResumeAnalyzerPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(backendUrl("/interview/evaluate"), {
        method: "POST",
        body: formData,
      });
  ...
  // Group interview questions by type for counts
  const questionGroups = data
    ? data.interview.questions.reduce(
        ...
        <div className="max-w-3xl mx-auto relative">
          ...
          >
            ...
            <div
              ...
              style={{
                borderColor: dragging ? "#8b5cf6" : resume ? "#34d399" : "rgba(139,92,246,0.35)",
                background:  dragging ? "rgba(139,92,246,0.1)" : resume ? "rgba(52,211,153,0.05)" : "rgba(139,92,246,0.04)",
              }}
            >
          ...
          {data && (
            <div className="space-y-4">
              ...
              {/* ── Resume Improvements (NEW) ── */}
              <Section title="Resume improvements" titleColor="#fbbf24" delay={0.42}>
                <div className="space-y-2">
                  {data.analysis.resumeImprovements.map(
                    (item: { section: string; issue: string; fix: string }, i: number) => (
              ...
              <Section title="Interview questions" titleColor="#818cf8" delay={0.45}>
                {/* Question type summary pills */}
                <div className="flex flex-wrap gap-2 mb-4">
                  {Object.entries(questionGroups).map(([type, count]) => (
                    <span
                      key={type}
                ...
                <div className="space-y-2">
                  {data.interview.questions.map((q, i: number) => (
